...
from app.deps import RequireAdmin, SessionDep, api_token_of, write_audit
...
async def _get_or_404(session: SessionDep, token_id: str) -> ApiToken:
...
async def rename_token(
    ...
    changed.
    """
    token = await _get_or_404(session, token_id)
    before = token.name
